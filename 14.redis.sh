...
# docker에 설치된 ewdis 접속명령어
docker exec -it eb8b325f3c8e redis-cli

# redis 는 0~15번 까지의 db로 구성(default 0번)
#db번호 선택
select db번호
...
set user:email:1 hong@naver.com (key = user:email:1)
set user:email:2 hong2@naver.com
# 이미 존재하는 key를 set 하면 덮어쓰기
set user:email:1 hong@naver.com 
set user:email:1 hong2@naver.com
=> get user:email:1 = hong2@aver.com
# key값이 이미 존재함녀 pass시키고 없을때만 set하기 위해서는 nx옵션 사용
set user:email:1 hong2@naver.com nx
...
#list 자료구조
#redis의 list는 deque와 같은 자료구조. 즉, double-endded-queue구조
lpush students kim1
lpush students lee1
rpush students park1

#list 조회
lrange students 0 2 #0번째부터 2번쨰까지
...
#빈번하게 변경되는 객체값을 저장시에는 hashes성능 효율적


#redis(데이터저장소),kafka(메시지저장소)

#redis
#1)pub/sub : 메시지 저장 X &&  전파 - > 성능빠름,메시지 유실 가능성 
#2)redis streams : 메시지 저장 &&전파 -> 메시지 유싱 가능성 X 성능느림